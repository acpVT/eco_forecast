...
**Answer Q.1**
Uncertainty in the context of forecasting is the idea that something about the way we are producing models is not perfect and it includes possible sources of error that may cause variations in forecasts and future outcomes or between multiple forecasts. 
...
**Answer Q.2**
Understanding uncertainty and realizing a range of possible outcomes helps managers because it allows for the establishment of plans that can capture the maximum and minimum changes that can happen as well as makes them aware that things in the future may change or be different than expected so plans may need to be adjusted. Understanding uncertainty before hand can help with making contingency plans for management.
...
source("assignment/R/plot_functions.R")
```
# Activity A:  
## Objective 1. Read in and visualize data from Lake Barco, FL, USA
...
**Answer Q.4**
Change in dissolved oxygen content and the associated changes in water chemistry often occur with rising water temperature. Communities of fish are also impacted given that many fish have a relativley small range of tolerable conditions.
...
```{r}
lake_df <- read_csv("assignment/data/BARC_airt_wtemp_celsius.csv", show_col_types = FALSE)
```
...
**Answer Q.5**
While there is an apparent relationship, I don't know if I would say it a particularly strong relationship. 
...
```{r}
weather_forecast <- read_csv("assignment/data/BARC_airt_forecast_NOAA_GEFS.csv", show_col_types = FALSE)
```
...
**Answer Q.6**
The multiple gray lines each represent a model run that created a forecast based on one ensemble member. This represents the driver uncertainty.
...
**Answer Q.7**
The water temperature is being predicted based on today's temperature, a forecasted temperature for tomorrows air temperature.
...
**Answer Q.8**
Water temp today (0.81) is more important than air temp tomorrow (0.18). Since the slope of the regression is greater for the water temperature today than the air temperature tomorrow, water temp scales the prediction more so than air temperature.
...
**Answer Q.9** 
Each of these indicate the model is performing well. The values fall within what we may consider to be a reasonable error and it means that the model fits the data well.
...
print("Predicted water temp = 15.9")
```
...
**Answer Q.11** 
Overall the pattern is similar between air and water temperature however there are differences in the first few days since the previous day's water temperatures are also a factor within the model.
...
**Answer Q.12**
By the end of the forecast, the distribution of possible futures is much larger than in the first two days indicating that the uncertainty increases throughout the forecast. 
...
**Answer Q.13** 
With the uncertainty, we can see that there are more possible futures than just one and so we cannot plan on there being just one certain outcome for the system.
...
**Answer Q.14**
If we have more data to use in fitting the model we can reduce parameter uncertainty by reducing the error in the fit.
...
**Answer Q.15**
Initial conditions uncertainty can be caused by inacurate measurments of the initial conditions. Things like having bad sensors, not enough sensors, unevenly or inappropriatley distributed sensors, or measurign at the wrong time can all increase the initial condition uncertainty. 
...
**Answer Q.16**
Parameter contributes the most throughout the forecast and increases with each day.
...
**Answer Q.17**
In general, I would believe that process uncertainty is a significant proportion of uncertainty in most models given that data availability is often a limiting component. It would also make sense that process uncertainty would contribute greatly since models are inherently simplifications of a system. I would expect that if we were looking at a larger system or a more open system (ie ocean or river), initial conditions uncertainty would play a bigger role given that it would be harder to take accurate measurments across the entire system.
...
```{r}
lake_df <- read_csv("assignment/data/BARC_lakedata.csv", show_col_types = FALSE)
```
...
```{r}
weather_forecast <- read_csv("assignment/data/BARC_forecast_NOAA_GEFS.csv", show_col_types = FALSE)
...
```{r}
fit <- lm(model_data$wtemp ~ model_data$wtemp_yday + model_data$airt + model_data$wnd)
...
for(d in 2:length(forecasted_dates)) {
  ...
  #run the model using parameter distributions instead of fixed values - REVISE THIS!!
    temp_pred$value <- param.df$beta1 + temp_lag$value * param.df$beta2 + temp_driv$air_temperature * param.df$beta3 + temp_driv$wind_speed * param.df$beta4
...
for(d in 2:length(forecasted_dates)) {
  ...
  #run model - REVISE THIS!!
    temp_pred$value <- coeffs[1] + temp_lag$value * coeffs[2] + temp_driv$air_temperature * coeffs[3] + 
      temp_driv$wind_speed * coeffs[4] + rnorm(30, mean = 0, sd = sigma)
