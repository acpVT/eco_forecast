...
BARC_past <- past_data |>
  ...
  TS_model <- BARC_past |>
    fabletools::model(tslm = TSLM(temperature ~ air_temperature + wind_speed + precipitation_flux))
...
LM_forecasts_EFI |> 
  ...
  geom_line() +
  facet_wrap(~site_id) +
  labs(title = paste0('Forecast generated for ', 'Temp', ' on ', LM_forecasts_EFI$reference_datetime[1]))
```

```{r write-forecast}
theme <- 'aquatics'
date <- LM_forecasts_EFI$reference_datetime[1]
forecast_name_1 <- paste0(LM_forecasts_EFI$model_id[1], ".csv")
...
write_csv(LM_forecasts_EFI, file.path('Forecasts',forecast_file_1))
