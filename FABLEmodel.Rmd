...
BARC_past <- past_data |>
  ...
  TS_model <- BARC_past |>
    fabletools::model(tslm = TSLM(temperature ~ air_temperature + wind_speed + total_precip))
  
  forecast <- TS_model |>  
    generate(h = forecast_starts$h, new_data = BARC_fut, bootstrap = T, times = 30) 
...
forecast_res <- generate(TS_model, new_data = BARC_past, bootstrap = T, times = 20)
lm_res <- lm(forecast_res$temperature ~ forecast_res$.sim)
summary(lm_res)
plot(forecast_res$temperature, forecast_res$.sim)
```
...
forecast_res <- generate(nn_model, new_data = BARC_past, bootstrap = T, times = 20)
lm_res <- lm(forecast_res$temperature ~ forecast_res$.sim)
summary(lm_res)
plot(forecast_res$temperature, forecast_res$.sim)
```
...
both <- bind_rows(NN_forecast_EFI, LM_forecasts_EFI)
